...
import { Pencil, CheckSquare, Coins, Briefcase } from "lucide-react";
...
export function EditDealDialog({ deal, organizations, dealStages, open, onOpenChange }: EditDealDialogProps) {
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            title: deal.title,
            value: deal.value,
            stage: deal.customStage || deal.stage,
            organizationId: deal.organizationId || "none",
        },
    });
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        if (result.success) {
            ...
            toast.error(result.error || "Failed to edit deal");
            ...
            <DialogContent className="sm:max-w-[550px] p-0 overflow-hidden bg-white/95 backdrop-blur-xl border border-zinc-200">
                <div className="bg-zinc-50/80 px-6 py-4 border-b border-zinc-200/60">
                    ...
                    <DialogDescription className="mt-1 text-sm text-zinc-500">
                        Modify the core details of this sales opportunity.
                    </DialogDescription>
                </div>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="px-6 py-4">
                        <div className="space-y-5">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-zinc-700 font-semibold">Deal Title</FormLabel>
                                        <FormControl>
                        ...
                        <DialogFooter className="mt-8 pt-4 border-t border-zinc-100 flex items-center sm:justify-between">
                            <span className="text-xs text-zinc-400 hidden sm:inline-block">Administrative Pipeline Modification</span>
