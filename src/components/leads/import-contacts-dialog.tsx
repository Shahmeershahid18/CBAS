        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                if (response.success) {
                    ...
                    setError("Failed to import contacts. " + response.error);
                }
                setLoading(false);
            },
            error: (error) => {
    ...
    return (
        <Dialog open={open} onOpenChange={(newOpen) => {
            ...
            <DialogTrigger asChild>
                <Button variant="outline" className="bg-background shadow-sm border-border text-foreground hover:bg-muted hover:text-foreground transition-all font-medium h-9">
            ...
            <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden bg-white/95 backdrop-blur-xl border border-zinc-200">
                <div className="bg-zinc-50/80 px-6 py-4 border-b border-zinc-200/60">
                    ...
                    <DialogDescription className="mt-1 text-sm text-zinc-500">
                        Upload a CSV file to bulk add contacts into your workspace.
                    </DialogDescription>
                </div>
                    ...
                    {!successMsg ? (
                        <div className="space-y-4">
                            ...
                            >
                                ...
                                <input id="contact-file-upload" type="file" accept=".csv" className="hidden" onChange={handleFileChange} />
                            </label>

                            <div className="flex justify-between items-center text-sm">
                                <a href={withBasePath("/Contact_Import_Template.csv")} download className="text-primary hover:underline font-medium">
