...
export function ImportLeadsDialog() {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Parse just to validate and count
    ...
    const handleImport = async () => {
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                const response = await importBulkLeads(leads);

                if (response.success) {
                    let msg = `Successfully imported ${response.count} leads.`;
                    if (response.duplicateCount && response.duplicateCount > 0) {
                        msg += ` (${response.duplicateCount} duplicates were skipped).`;
                    }
                    setSuccessMsg(msg);
                    router.refresh();
                    setTimeout(() => {
                    ...
                    setError("Failed to import leads. " + response.error);
                }
                setLoading(false);
            },
            error: (error) => {
    ...
    return (
        <Dialog open={open} onOpenChange={(newOpen) => {
            ...
            <DialogTrigger asChild>
                <Button variant="outline" className="w-full xs:w-auto bg-background shadow-sm border-border text-foreground hover:bg-muted hover:text-foreground transition-all font-medium h-9">
                    <UploadCloud className="mr-2 w-4 h-4 text-muted-foreground" />
                    Import <span className="hidden xs:inline ml-1">Leads</span>
                </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden bg-white/95 backdrop-blur-xl border border-zinc-200">
                <div className="bg-zinc-50/80 px-6 py-4 border-b border-zinc-200/60">
                    ...
                    <DialogDescription className="mt-1 text-sm text-zinc-500">
                        Upload a CSV file to bulk add leads into your pipeline.
                    </DialogDescription>
                </div>
