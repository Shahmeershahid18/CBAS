    ...
    const handleImport = async () => {
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                const response = await importBulkOrganizations(orgs);

                if (response.success) {
                    setSuccessMsg(`Successfully imported ${response.count} organizations.`);
                    setTimeout(() => {
                        setOpen(false);
                        resetState();
                    }, 2000);
            ...
            <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden bg-white/95 backdrop-blur-xl border border-zinc-200">
                <div className="bg-zinc-50/80 px-6 py-4 border-b border-zinc-200/60">
                    ...
                    <DialogDescription className="mt-1 text-sm text-zinc-500">
                        Upload a CSV file to bulk add companies.
                    </DialogDescription>
                </div>
                    ...
                    {!successMsg ? (
                        <div className="space-y-4">
                            ...
                            >
                                ...
                                <input id="file-upload-org" type="file" accept=".csv" className="hidden" onChange={handleFileChange} />
                            </label>
